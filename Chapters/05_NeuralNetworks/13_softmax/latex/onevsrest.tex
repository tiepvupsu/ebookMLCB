...
\begin{tikzpicture}[shorten >=1pt,draw=black,thick, node distance=\layersep, >= stealth]
    ...
    \foreach \source in {1,...,4}
        % \foreach \dest in {1,...,4}
            \draw [thick, ->](O-\source.east)  --  (O2-\source.west);
   
    \node at (2.5*\layersep, -2) { 1 vs  rest};
    \node at (2.5*\layersep, -3.5) { 2 vs  rest};
    \node at (2.5*\layersep, -6.5) { C vs  rest};
   \begin{scope}[xshift = 9cm]
    ...
    \node [text width= 4cm, anchor = west] at (-1, \offset - 4*\dd) {$a_i = \text{sigmoid}(z_i) \in \R$};
    \node [text width= 4cm, anchor = west] at (-1, \offset - 5.2*\dd) {$0 < a_i < 1$};
   \end{scope}
   ...
   %      \node [anchor = west, align = left, scale = 1.5, below= 0mm of I-7.south] {$\mathbf{x} (\mathbf{\bar{x}})$};
   %      \node [anchor = west, align = left, scale = 1.5, below= 1mm of O2-4.south] {$\mathbf{a}$};

   %      \node [draw] at (1, 0) {$\mathbf{a} = \text{softmax}(\mathbf{W}^T \mathbf{\bar{x}})$};
   %      % \node [scale = 1.5, below= 1mm of I-7.south] {$\mathbf{z = \text{softmax}(\)}$};

   %      \draw [scale = 2, ->, ultra thick] (-1.4, -2.1)  --  node[above] {short form} ++(1, 0);
   % \end{scope}
\end{tikzpicture}
% End of code
\end{document}
